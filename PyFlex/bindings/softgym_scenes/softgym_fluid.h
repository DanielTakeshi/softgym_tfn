...
public:
	...
	void Initialize(py::array_t<float> scene_params, int thread_idx = 0)
	{
		...
		*/
		...
		// g_drawEllipsoids = true;
		// g_drawPoints = false;
		// NOTE(daniel) copying from softgym_mixed_media.h, this is necessary
		// to get the depth camera to work.
		bool hasFluids = true;
    ...
    if (g_render) {
			AddSensor(cam_width, cam_height, 0,
                Transform(Vec3(cam_x, cam_y, cam_z),
                          rpy2quat(cam_angle_x, cam_angle_y, cam_angle_z)),
                DegToRad(45.f), hasFluids, p);
		}
	}
