...
bool g_drawMesh;
bool g_drawCloth;
bool g_drawFluids = true; // Daniel: actually I just use drawEllipsoids
...
void DrawSensors(const int numParticles, const int numDiffuse, float radius, Matrix44 lightTransform)
{
	for (int i = 0; i < g_renderSensors.size(); i++)
	{
		...
		if (sensor.parent)
		{
      throw "Sensor can't have parent in PyFlex!";
    ...
    DrawShapes();
		...
		// draw fluids (Daniel: adding `g_drawFluids` flag)
		if (sensor.fluidRendererId != -1 && g_drawFluids) {
