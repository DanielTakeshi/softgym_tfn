...
void BindSolidShader(Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap, float bias, Vec4 fogColor)
{
	...
	if (s_diffuseProgram)
	{
		...
		glVerify(glUniform1f(glGetUniformLocation(s_diffuseProgram, "depthMinRange"), currentDepthProfile.minRange));
		glVerify(glUniform1f(glGetUniformLocation(s_diffuseProgram, "depthMaxRange"), currentDepthProfile.maxRange));
...
void DemoContextOGL::renderEllipsoids(FluidRenderer* renderer, FluidRenderBuffers* buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug, const RenderTexture* customRenderTarget)
{
	OGL_Renderer::RenderEllipsoids(renderer, buffers, n, offset, radius, screenWidth, screenAspect, fov, lightPos, lightTarget, lightTransform, shadowMap, color, blur, ior, debug, customRenderTarget);
}
...
{
  OGL_Renderer::SetDepthRenderProfile(profile);
}

void DemoContextOGL::setRenderTarget(const RenderTexture* target, int x, int y, int width, int height)
{
  OGL_Renderer::SetRenderTarget(target, x, y, width, height);
}

RenderTexture* DemoContextOGL::createRenderTarget(int width, int height, bool depth)
{
  return OGL_Renderer::CreateRenderTarget(width, height, depth);
}

void DemoContextOGL::readRenderTarget(const RenderTexture* target, float* rgbd, int x, int y, int width, int height)
{
  OGL_Renderer::ReadRenderTarget(target, rgbd, x, y, width, height);
}

void DemoContextOGL::destroyRenderTexture(RenderTexture* t)
{
  OGL_Renderer::DestroyRenderTexture(t);
}
