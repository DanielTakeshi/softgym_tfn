...
class DemoContext
{
	...
	virtual void renderEllipsoids(FluidRenderer* renderer, FluidRenderBuffers* buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ::ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug, const RenderTexture* customRenderTarget = 0) = 0;
  ...
  virtual void setRenderTarget(const RenderTexture* target, int x, int y, int width, int height) = 0;
  virtual RenderTexture* createRenderTarget(int width, int height, bool depth) = 0;
  virtual void readRenderTarget(const RenderTexture* target, float* rgbd, int x, int y, int width, int height) = 0;
  virtual void destroyRenderTexture(RenderTexture* t) = 0;
