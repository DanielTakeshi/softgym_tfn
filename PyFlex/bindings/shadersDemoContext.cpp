...
void SetDepthRenderProfile(DepthRenderProfile profile) { return s_context->setDepthRenderProfile(profile); }

void SetRenderTarget(const RenderTexture* target, int x, int y, int width, int height) { return s_context->setRenderTarget(target, x, y, width, height); }

RenderTexture* CreateRenderTarget(int width, int height, bool depth) { return s_context->createRenderTarget(width, height, depth); }

void ReadRenderTarget(const RenderTexture* target, float* rgbd, int x, int y, int width, int height) { return s_context->readRenderTarget(target, rgbd, x, y, width, height); }

void DestroyRenderTexture(RenderTexture* t) { return s_context->destroyRenderTexture(t); }
...
void RenderEllipsoids(FluidRenderer* renderer, FluidRenderBuffers* buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug, const RenderTexture* customRenderTarget)
{
	s_context->renderEllipsoids(renderer, buffers, n, offset, radius, screenWidth, screenAspect, fov, lightPos, lightTarget, lightTransform, shadowMap, color, blur, ior, debug, customRenderTarget);
}
