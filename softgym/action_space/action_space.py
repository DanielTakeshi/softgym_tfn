...
class Picker(ActionToolBase):
    def __init__(self, num_picker=1, picker_radius=0.05, init_pos=(0., -0.1, 0.), picker_threshold=0.00625, particle_radius=0.05,
...
class PickerPickPlace(Picker):
    def __init__(self, num_picker, env=None, picker_low=None, picker_high=None, delta_move=0.02, **kwargs):
        ...
        self.delta_move = delta_move  # By default this is 0.02
        self.env = env
...
class PickerQPG(PickerPickPlace):
    def __init__(self, image_size, cam_pos, cam_angle, full=True, qpg_delta_move=0.007, qpg_max_dist=0.3, **kwargs):
        kwargs['delta_move'] = qpg_delta_move
        super().__init__(**kwargs)
        self.image_size = image_size
        self.cam_pos = cam_pos
        self.cam_angle = cam_angle
        self.action_space = Box(np.array([-1., -1, *(-qpg_max_dist, 0, -qpg_max_dist)]),
                                np.array([1., 1., *([qpg_max_dist] * 3)]), dtype=np.float32)
    ...
    def step(self, action):
        ...
        picker_pos, particle_pos = self._get_pos()
        picker_pos[0, :3] = st_high[:3]
        self._set_pos(picker_pos, particle_pos)
