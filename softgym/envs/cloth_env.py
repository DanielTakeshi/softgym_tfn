...
class ClothEnv(FlexEnv):
    def __init__(self, observation_mode, action_mode, num_picker=2, render_mode='particle', picker_radius=0.05, \
            picker_threshold=0.005, particle_radius=0.00625, qpg_delta_move=0.02, **kwargs):
        ...
        if action_mode.startswith('key_point'):
            ...
            self.action_tool = PickerQPG((self.camera_height, self.camera_height), cam_pos, cam_angle, picker_radius=picker_radius,
                                         num_picker=num_picker, particle_radius=particle_radius, env=self,
                                         picker_low=(-0.3, 0., -0.3), picker_high=(0.3, 0.3, 0.3), qpg_delta_move=qpg_delta_move
                                         )
            self.action_space = self.action_tool.action_space
    ...
    def _sample_cloth_size(self):
        ...
        # return np.random.randint(28, 33), np.random.randint(48, 53)
        return np.random.randint(47, 48), np.random.randint(47, 48)
        # return np.random.randint(42, 43), np.random.randint(42, 43)
    ...
    def get_default_config(self):
        ...
        if self.action_mode in ['sawyer', 'franka']:
            ...
            # cam_pos, cam_angle = np.array([-0.0, 2.82, 0.82]), np.array([0, -45 / 180. * np.pi, 0.])

        config = {
            'ClothPos': [-1.6, 2.0, -0.8],
            'ClothSize': [int(0.6 / particle_radius), int(0.368 / particle_radius)],
            'ClothStiff': [0.8, 1, 0.9],  # Stretch, Bend and Shear
            'camera_name': self.camera_name, #'default_camera',
            'camera_params': {'default_camera':
                              ...
                              'camera_for_RSS_plan_plot':{
                                  'pos': np.array([0, 1.5, 0]),
                                  'angle': np.array([0, -90 / 180 * np.pi, 0]),
                                  'width': self.camera_width,
                                  'height': self.camera_height
                              },
                              'camera_for_RSS_edge_plot':{
                                  'pos': np.array([-0.0, 0.32, 0.32]),
                                  'angle': np.array([0, -45 / 180. * np.pi, 0.]),
                                  'width': self.camera_width,
                                  'height': self.camera_height
                              },
                              'camera_for_RSS_open_loop_plot':{
                                  'pos': np.array([-0.0, 0.35, 0.35]),
                                  'angle': np.array([0, -45 / 180. * np.pi, 0.]),
                                  'width': self.camera_width,
                                  'height': self.camera_height
                              },
