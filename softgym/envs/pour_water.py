...
class PourWaterPosControlEnv(FluidEnv):
    ...
    def __init__(self, observation_mode, action_mode, real_world_scale=False,
        ...
        Class attributes:
          (can change from agent)
            ...
            glass_y: box y coord (starts at 0 if rotation_bottom, positive moves up)
                Does not change if only rotating
            glass_rotation: box rotation (starts at 0, positive dumps in +x)
          (fixed per episode)
        ...
        if observation_mode in ['key_point_1', 'key_point_2', 'key_point_3', 'key_point_4']:
            ...
            obs_dim = 22  # See `get_obs()` for more.
            self.observation_space = Box(
                    low=np.array([-np.inf] * obs_dim),
                    high=np.array([np.inf] * obs_dim),
                    dtype=np.float32)
    ...
    def initialize_camera(self):
        ...
        if not self.real_world_scale:
            self.camera_params = {
                ...
                #                 'width': self.camera_width,
                #                 'height': self.camera_height},
                # A good view for seed 0 if we generate it and want a sideways view.
                # (I did this to generate a sideways flow figure for the pull figure.)
                'pull_figure': {'pos': np.array([0.60, 0.80, 1.10]),
                                 'angle': np.array([15.*DEG_TO_RAD,
                                                    -30.*DEG_TO_RAD,
                                                    0.*DEG_TO_RAD]),
                                 'width': self.camera_width,
    ...
    def _get_obs(self):
        ...
        08/17/2022: extract depth image as the 6th item in the combo obs.
        """
        def get_water_info():
        ...
        if self.observation_mode == 'cam_rgb':
            ...
            # 08/25/2022: now depth_segm for another baseline. For this env, use 4
            # channels: depth, tool, target cup, water. Get all masks to be {0,1}.
            # TODO(daniel): how are we dealing with water? For now I am going to assume
            # there can be overlap, which we do have in the segmented point cloud. In the
            # segmented PCL we assume we have g.t. water particles and that both boxes are
            # not occluded. So for that let's do the same thing here but we will have the
            # segmentation masks set so the mask for water might overlap with the others.
            # NOTE(daniel): current template is copied from Mixed Media, need to fix!
            # NOTE(daniel): actually now I feel we have to get water particles now :(
            cam_rgb = self.get_image(self.camera_width, self.camera_height)
            ...
            # Actually for now we're just taking the keypoints plus info about the water.
            pose_info = get_keypoints()
            water_info = get_water_info()
            #boxes = np.concatenate((pose_info[:,:3], pose_info[:,6:10]), axis=1).flatten()
            #state = np.concatenate((water_info, boxes))  # dim 72
            ...
            #     cup_state = np.array([self.glass_x, self.glass_y, self.glass_rotation, self.glass_dis_x, self.glass_dis_z,
            #                         self.glass_distance, self.poured_height, self.poured_glass_dis_x, self.poured_glass_dis_z])
                ...
                cup_state = np.array([self.glass_distance - self.glass_x, self.poured_height - self.glass_y, self.glass_rotation,
    ...
    def _step(self, action):
        ...
        if self.action_mode == 'rotation_bottom':
            ...
            move = action[:2]
            rotate = action[5]  # assume the last one
        else:
        ...
        x = self.glass_x + dx
        y = self.glass_y + dy
        theta = self.glass_rotation + dtheta
        ...
        if (not self.judge_glass_collide(new_states, theta) and
                self.above_floor(new_states, theta)):
    ...
    def rotate_glass(self, prev_states, x, y, theta):
        ...
        x, y, theta: the current configuration of the glass, we continually track this
            ...
            the updated values.
        """
        axis_ang = np.array([0., 0., -1.])
        dis_x, dis_z = self.glass_dis_x, self.glass_dis_z
        quat_curr = quatFromAxisAngle(list(axis_ang), theta)
        border = self.border
        x_center = x
        ...
        if (self.action_mode in ['rotation_bottom', 'translation_axis_angle']):
            ...
            states[1, :3] = rotate_rigid_object(center=rotate_center, axis=axis_ang, angle=theta, relative=relative_coord)

            # right wall
            relative_coord = np.array([(dis_x + border) / 2., (self.height + border) / 2., 0.])
            states[2, :3] = rotate_rigid_object(center=rotate_center, axis=axis_ang, angle=theta, relative=relative_coord)

            # back wall
            relative_coord = np.array([0, (self.height + border) / 2., -(dis_z + border) / 2.])
            states[3, :3] = rotate_rigid_object(center=rotate_center, axis=axis_ang, angle=theta, relative=relative_coord)

            # front wall
            relative_coord = np.array([0, (self.height + border) / 2., (dis_z + border) / 2.])
            states[4, :3] = rotate_rigid_object(center=rotate_center, axis=axis_ang, angle=theta, relative=relative_coord)
    ...
    def _print_glass(self):
        print(f'Glass: ({self.glass_x:0.3f}, {self.glass_y:0.3f}, {self.glass_rotation:0.3f})')
...
class PWAlgorithmicPolicy():
    ...
    def get_action(self, step, version):
        ...
        same_rot = self._prior[2] == self.env.glass_rotation
        ...
        self._prior[2] = self.env.glass_rotation
        ...
        if self.state == 0:
            ...
            if np.abs(x_thresh - poured_center[0]) < 0.02:
                # This is just to make a nonzero action to avoid a
                # jump in the next stage.
                dx = 0.0005
                self.state += 1
            elif x_thresh < poured_center[0]:
                dx = 0.003
            ...
            if 90.0 <= (self.env.glass_rotation * RAD_TO_DEG) <= 95.0:
                dx = 0.001
            # At this point, should be done.
            if (self.env.glass_rotation * RAD_TO_DEG) >= 120.0:
                self.state += 1
        elif self.state == 4:
            # Rotate back to neutral! (This is more of a qualitative thing,
            # can choose to ignore this if desired.)
            if (self.env.glass_rotation * RAD_TO_DEG) <= 5.0:
                self.state += 1
            elif collision:
                dy = 0.003
            else:
        ...
        if version == 1:
            ...
            action = np.array([dx, dy, 0, 0, 0, drot])
        else:
            raise NotImplementedError(version)

        if self._print:
            glass_rot = self.env.glass_rotation * RAD_TO_DEG
