...
class PassWater1DEnv(FluidEnv):
    ...
    def __init__(self, observation_mode, action_mode, cached_states_path='pass_water_init_states.pkl', **kwargs):
        """This class implements a pouring water task named `PassWater`.
    ...
    def get_default_config(self):
        """(Daniel) Get configuration and mostly visually check if the physics looks good.

        Not sure why but the public version uses slightly different parameters:
        ...
        Then why do we not have a 2:1 ratio of radius:rest_dist_coef? Seems like it's reversed.
        TODO(daniel) What are the units here?
        TODO(daniel) Why does changing the fluid radius make the box (glass?) bigger? I see the
            code for that here, it's just a bit strange why it would be needed. In any case, we
            probably want a larger box than what's provided in default SoftGym.
        """
        config = {
            'fluid': {
    ...
    def set_scene(self, config, states=None):
        ...
        if states is None:
            ...
            _i = 0  # NOTE(daniel): I've seen cases when `not_total_num` remains > 0 perpetually.
            while not_total_num > 0 and _i < 4:
                _i += 1
    ...
    def init_glass_state(self, x, y, glass_dis_x, glass_dis_z, height, border):
        ...
        Note the distinction between current and previous shape positions and rotations,
        must get ordering correct. Since this is initialization, "last_y" is set to 0.
        Also, the glass doesn't rotate, so just set a consistent rotation.
        '''
